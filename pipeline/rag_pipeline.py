...
class RAGPipeline:
    def __init__(self, config, vector_store):
        ...
        self.vector_store = vector_store   # <-- ВОТ ЭТО ОБЯЗАТЕЛЬНО
    ...
    def generate(self, prompt: str) -> str:
        payload = {
            ...
            "prompt": prompt,
            "stream": False,
            # options можешь оставить как есть
        }

        s = requests.Session()
        s.trust_env = False  # <-- ВАЖНО: игнорировать HTTP_PROXY и т.п.

        r = s.post(self.ollama_url, json=payload, timeout=300)
        if not r.ok:
    ...
    def query(self, question: str):
        ...
        return {
            "answer": answer,
            "contexts": [
                {"chunk_id": cid, "score": float(score), "text": text}
                for cid, score, text in contexts
            ],
        }
